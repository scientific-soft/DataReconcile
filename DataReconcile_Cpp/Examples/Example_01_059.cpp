...
VectorXd my_func(const VectorXd& x, const VectorXd& params) {
    VectorXd result(2);
    result << params(0) * x(0) - x(1), x(0) - x(1) / params(0);
    return result;
}

int main() {
    VectorXd vals_for_reconc(2);
    vals_for_reconc << 1.0, 2.0;

    VectorXd params(1);
    params << 2.0;

    MatrixXd cov_matrix(2, 2);
    cov_matrix << 0.1, 0.0, 0.0, 0.2;

    string ChosenMode = "LS";

    auto result = estAccuracyIncreaseByDR(my_func, ChosenMode, vals_for_reconc, params, cov_matrix);

    cout << "Accuracy Increase Ratio: " << result.first.transpose() << endl;
    cout << "Variances of DR Result: " << result.second.transpose() << endl;
