...
VectorXd DRnonparamEqIneqRobust(const std::function<VectorXc(const VectorXc&, const VectorXd&)>& eqies_model,
    const std::function<VectorXc(const VectorXc&, const VectorXd&)>& ineqies_model,
...
int main() {
    ...
    VectorXd reconciled_data = DRnonparamEqIneqRobust(eqies_model, ineqies_model, msrd_data, model_params, prior_vars, bandwidths);
