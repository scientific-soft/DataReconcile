...
VectorXd DRsemiparamEqRobust(const function<VectorXd(const VectorXd&, const VectorXd&)>& depend_func,
    const MatrixXd& msrd_data,
    ...
    for (size_t j = 0; j < xNum; ++j) {
        ...
        error_params[j] = median_absolute_deviation(column) * 10.0;
    }
    // -----------------------------------------
...
int main() {
    try {
        ...
        VectorXd reconciled_data = DRsemiparamEqRobust(example_depend_func, msrd_data, error_params, alpha_params, params);
