...
VectorXd DRparamEqRobust(DependFunc depend_func,
    const MatrixXd& msrd_data,
...
int main() {
    try {
        ...
        error_params << 200.0, 2.0;
        VectorXd params(1);
        params << 0;

        VectorXd reconciled_data = DRparamEqRobust(example_depend_func, msrd_data, error_params, params);
