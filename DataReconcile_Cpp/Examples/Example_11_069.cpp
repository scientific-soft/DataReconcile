...
// Function to reconcile measured data using the Alpha Gram-Charlier method
VectorXd DRsemiparamEqIneq(const std::function<VectorXc(const VectorXc&, const VectorXd&)>& eqies_model,
    const std::function<VectorXc(const VectorXc&, const VectorXd&)>& ineqies_model,
    ...
    AlphaSystemFunctor functor(eqies_model, ineqies_model, msrd_data, error_params, alpha_params, params_, xNum, n, eqNum, ineqNum);
...
int main() {
    ...
    VectorXd reconciled_data = DRsemiparamEqIneq(eqies_model, ineqies_model, msrd_data, error_params, alpha_params, params_);
