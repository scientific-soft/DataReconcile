function reconciled = DRparamEqIneq(eqFunc, ineqFunc, msrd_data, error_params, model_params)
    ...
    mu_and_l_res = fsolve(@SystemToSolve, mu_and_l_start, [], ...
      eqFunc, ineqFunc, msrd_data, error_params, model_params, xNum, n, eqNum, ineqNum);
...
function mustbezeros = SystemToSolve(mu_and_l, eqFunc, ineqFunc, ...
    ...
    mustbezeros = zeros([xNum+eqNum+ineqNum, 1]);
    for j = 1 : xNum
        if vars(j) == 0
