function reconciled = DRparamEqRobust(Func, msrd_data, error_params, params_)
    ...
    l = zeros([length(Func(msrd_data, params_)), 1]);
    ...
    mu_and_l_res = fsolve(@(mu_and_l_start) SystemToSolve (mu_and_l_start, ...
      Func, msrd_data, error_params, params_), mu_and_l_start);
