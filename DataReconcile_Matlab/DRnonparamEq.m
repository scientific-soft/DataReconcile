function reconciled_with_Kernels = DRnonparamEq(Func, msrd_data, model_params, prior_vars, bandwidths)
...
% bandwidths - вектор значений ширины 
    l = zeros([length(Func(mean(msrd_data,2), model_params)), 1]);
    ...
    mu_and_l_est = fsolve(@KernelsSystemToSolve, mu_and_l_start, [], ...
              Func, msrd_data, model_params, bandwidths, prior_vars);
